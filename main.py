...
# this line is converting the FastMCP server to web facing, so it can be mounted into the FastAPI app below.
...
# if we want to run as stdio MCP server . In MCP inspector - CLIENT_PORT=8080 npx @modelcontextprotocol/inspector python3 main.py
# if __name__ == "__main__":
#     mcp.run()

# if we want to run as FastAPI server . In browser - http://localhost:8000
# uv run uvicorn main:app --reload

#if you want to run only mcp server in http mode using uv -
